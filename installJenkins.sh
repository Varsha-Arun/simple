...
get_api_token() {
        curl -s --cookie-jar /tmp/cookies -u $USER:$PASSWORD $URL/crumbIssuer/api/json
        curl -X POST -H 'Jenkins-Crumb:'$JENKINS_CRUMB --cookie /tmp/cookies $URL/me/descriptorByName/jenkins.security.ApiTokenProperty/generateNewToken?newTokenName=testtoken -u $USER:$PASSWORD
}
...
confirm_jenkins_url
install_artifactory_plugin
#JENKINS_API_TOKEN=get_api_token
echo "JENKINS_API_TOKEN=XXXX"
